...
const AUTH_API="http://localhost:8082/api/v1.0/tweets/";
...
export class JwtAuthService {
  ...
  jwtAuthRequest(){
    ...
    return this.http.post<String>("http://localhost:8082/authenticate",this.authRequest,{ headers, responseType: 'text' as 'json' })
    .pipe(
  ...
  getAllUsers():Observable<User[]>{
    let headers=new HttpHeaders(
     ...
     return this.http.get<User[]>("http://localhost:8082/api/v1.0/tweets/users/all",{headers})  
  }
  ...
  likeATweet(userId,tweetId){
    let headers=new HttpHeaders(
      ...
      return this.http.put<Tweets[]>("http://localhost:8082/api/v1.0/tweets/"+userId+"/like/"+tweetId,{headers})
  }
